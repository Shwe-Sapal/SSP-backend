...
export const getPurchaseOverallReport = asyncErrorHandler(
  async (req, res, next) => {
    ...
    res.status(200).json({
      ...
      data: {
        totalPurchaseValue: Math.round((summary.totalPurchaseValue || 0) * 100) / 100,
        totalOrders: summary.totalOrders || 0,
        averageValue: Math.round((summary.averageValue || 0) * 100) / 100,
        totalReturnAmount: Math.round((returnSummary.totalReturnAmount || 0) * 100) / 100,
        totalReturnsCount: returnSummary.totalReturnsCount || 0,
        netPurchaseValue: Math.round(netPurchaseValue * 100) / 100,
        statusBreakdown,
        topSuppliers,
      },
    });
  ...
  async (req, res, next) => {
    ...
    let formattedProducts = productsAgg.map(p => {
       ...
       return {
          ...
          quantityPurchased: Math.round(trueQty * 1000) / 1000,
          totalCost: Math.round((p.totalCost || 0) * 100) / 100,
...
export const getPurchaseReturnsReport = asyncErrorHandler(
  async (req, res, next) => {
    ...
    res.status(200).json({
      ...
      data: {
        summary: {
          totalGrossPurchase: Math.round(totalGrossPurchase * 100) / 100,
          totalPOCount,
          totalReturnAmount: Math.round(totalReturnAmount * 100) / 100,
          totalReturnsCount,
          netPurchaseValue: Math.round(netPurchaseValue * 100) / 100,
          totalRefundedAmount: Math.round(totalRefundedAmount * 100) / 100,
          totalExchangeValue: Math.round(totalExchangeValue * 100) / 100,
          totalExchangeQuantity: Math.round(totalExchangeQuantity * 1000) / 1000,
          totalRecoveredValue: Math.round(totalRecoveredValue * 100) / 100,
          recoveryRate: Math.round(recoveryRate * 100) / 100,
          pendingReturnsCount,
          pendingReturnAmount: Math.round(pendingReturnAmount * 100) / 100,
          completedReturnsCount,
        },
        reasonBreakdown,
        supplierBreakdown,
